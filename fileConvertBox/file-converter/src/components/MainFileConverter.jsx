...
const MainFileConverter = () => {
  ...
  const getTheFile = (e) => {
    ...
    setfiles(file);
    const change = URL.createObjectURL(file);
        ...
        >
          ...
          <span>
            <form className="absolute flex  items-center w-[25%]  justify-center  fileform h-[80px] top-[0px] left-[38%] h-[90px] ">
              <input onChange={getTheFile} type="file" className=" w-full h-full inputfile" />
            </form>
            <ArrowUpTrayIcon className="h-[35px] uploadarrow" />
          </span>
        </div>
        <h5 className="text-gray-200 mt-3 font-light ">{image !== '' ? image :'upload file'}</h5>
        <button className="mt-5 text-gray-600 px-8 py-3 rounded-md convertbtn " onClick={SendFile}>{loading? 'loading...':'Convert'}</button>
      </div>
    </div>
  );
};
